...
def get():
    ...
    conn = psycopg2.connect(host='127.0.0.1', user="postgres", password="123", dbname='postgres', cursor_factory=RealDictCursor)
    cur = conn.cursor()      
...
def get3():
    ...
    conn = psycopg2.connect(host='127.0.0.1', user="postgres", password="123", dbname='postgres', cursor_factory=RealDictCursor)
    cur = conn.cursor()      
    cur.execute("select id, iddepart,name,id_mananger  from team")
    result = cur.fetchall()
    result2 = {"team":[]}
    for i in result:
        result2["team"].append(i)
    print(result2)
#    dict([result])
    print(type(result))
#    return jsonify(result)
...
def writer():
    ...
    conn = psycopg2.connect(host='127.0.0.1', user="postgres", password="123", dbname='postgres')
    cur = conn.cursor()
    for i in result:
        print(i)
...
def writer2():
    ...
    if str(result)=="1":
        ...
        if task == "write":
            cur.execute("insert into employee (team_id,name,surname,experience,position,salary,coef) values ('"+ str(result["team_id"])+"','"+ str(result["name"])+"','"+ str(result["sname"])+"','"+ str(result["exp"])+"','"+ str(result["position_id"])+"','"+ str(result["salary"])+"','"+ str(result["coefficient"])+"')")
            conn.commit()
...
def writer3():
    ...
    #        else: task = "write"
    #    if task == "write":
    cur.execute("insert into team (iddepart,name) values ('"+ str(result["depart_id"])+"','"+ str(result["name"])+"')")
    conn.commit()
